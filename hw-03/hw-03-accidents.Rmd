...
1.  How many observations (rows) does the dataset have? Instead of hard coding the number in your answer, use inline code. 768


2.  Run `View(accidents)` in your Console to view the data in the data viewer. What does each row in the dataset represent? 
Each row represents a single recorded road traffic accident, including information about its location, time, severity, vehicles involved, and casualties.
...
ggplot(data = accidents, aes(x = time, fill = severity)) +
  ...
  labs(
    ...
    subtitle = "Comparing weekday vs weekend accidents",
    x = "Time of Day",
    y = "Density",
    fill = "Severity"
  ) +
  scale_x_time(breaks = hms::as_hms(c("00:00:00", "06:00:00", "12:00:00", "18:00:00", "24:00:00")),
...
On weekdays, accidents peak sharply around 17:00–18:00, likely reflecting rush hour traffic, with a smaller morning peak around 08:00–09:00. On weekends, the distribution is broader with a later single peak around 17:00–18:00 and no morning rush hour spike. Fatal accidents appear more evenly spread throughout the day on weekdays, while slight accidents dominate the evening peak in both weekday and weekend patterns.
...
# Your code here
accidents %>%
  filter(light %in% c("Daylight", "Darkness: lights lit", "Darkness: no lighting")) %>%
  filter(road_type %in% c("Single carriageway", "Dual carriageway", "Roundabout")) %>%
  ggplot(aes(x = road_type, fill = severity)) +
  geom_bar(position = "fill") +
  facet_wrap(~light, ncol = 3) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Accident Severity by Road Type and Light Conditions",
    subtitle = "Proportion of severity levels across road and lighting types",
    x = "Road Type",
    y = "Proportion",
    fill = "Severity"
  )
...
Does the proportion of fatal and serious accidents vary by road type depending on lighting conditions?

**What variables were used:** 

severity, road_type, and light

**Interpretation (3-4 sentences):** What patterns or insights does your visualization reveal?

In daylight conditions, all types of roads experience slight accidents, with roundabouts experiencing the least number of serious accidents. Dual carriageways experience a higher proportion of serious accidents on the roundabouts. In darkness without lighting, there is a noticeable increase in the proportion of both fatals and serious accidents on single and dual carriageways. Thus, both the type of road and the lighting of that road contribute to the likelihood of accidents of that severity occurring on those roads. The combination of driving on a road of this type without lighting makes for the most dangerous road condition.
