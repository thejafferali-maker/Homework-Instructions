...
Majors with higher proportions of women tend to have lower median incomes. STEM majors cluster toward lower proportions of women and higher incomes, while non-STEM majors show the opposite pattern, suggesting a possible relationship between gender composition and earnings.

```{r women-income-stem, eval = TRUE}
ggplot(data = college_recent_grads, aes(x = sharewomen, y = median, color = major_type)) +
  ...
  labs(
    ...
    y = "Median Income",
    color = "Major Type",
    title = "Median Income vs. Proportion of Women by Major Type"
  )
```
...
college_recent_grads %>%
  group_by(major_category) %>%
  summarise(avg_sharewomen = mean(sharewomen, na.rm = TRUE)) %>%
  arrange(desc(avg_sharewomen))
```

**Your findings (3-4 sentences):**

The proportion of women varies substantially across major categories, with Education and Psychology & Social Work having the highest average share of women. In contrast, Engineering and Computers & Mathematics have the lowest proportions of women. This aligns with well-documented gender gaps in technical fields. These differences in gender composition may also partly explain the income disparities observed across major categories in earlier exercises.
