...
ggplot(comb_name_props, aes(x = cat_prop, y = dog_prop)) +
  ...
  xlim(0.002, 0.01) +
  ylim(0.002, 0.01) +
 annotate("text", x = 0.003, y = 0.009, label = "🐶", size = 5) +
annotate("text", x = 0.009, y = 0.003, label = "🐱", size = 5)

```

a.  What names are more common for cats than dogs? The ones above the line or the ones below the line?

Names that are more common for cats are the ones below the line
...
The relationship is positive. This means that names that are common for cats also tend to be common for dogs.
